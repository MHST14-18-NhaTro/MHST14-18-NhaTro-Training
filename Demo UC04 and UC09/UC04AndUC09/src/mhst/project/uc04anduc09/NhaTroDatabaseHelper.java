...
public class NhaTroDatabaseHelper extends SQLiteOpenHelper {
	...
	private final String createTable = "create table if not exists "
			+ tableNhaTro + " (" + colID + " integer not null, " + colTenNhaTro
			+ " text not null, " + colDiaChi + " text not null, " + colGiaPhong
			+ " integer not null, " + colSoPhong + " integer not null, "
			+ colDienTichPhong + " double, " + colOGhep + " tinyint, "
			+ colThongTinThem + " text, " + colHinhAnh + " blob, "
			+ colTenChuTro + " text not null, " + colSoDienThoai
			+ " text not null, " + colEmail + " text, " + "primary key("
			+ colID + ")" + ");";
	...
	public void themMotSoNhaTro() {
		NhaTro nhaTro1 = new NhaTro("nhà trọ 1", "địa chỉ 1", 1, 1, 1, false,
				"đây là nhà trọ demo 1", null, "chủ trọ 1", "111", "email@1");
		themNhaTro(nhaTro1);
		NhaTro nhaTro2 = new NhaTro("nhà trọ 2", "địa chỉ 2", 2, 2, 2, true,
				"đây là nhà trọ demo 2", null, "chủ trọ 2", "222", "email@2");
		themNhaTro(nhaTro2);
	}
	...
	public void themNhaTro(NhaTro nhaTro) {
		...
		value.put(colHinhAnh, getBlobFromBitmap(nhaTro.getHinhAnh()));
		db.insert(tableNhaTro, null, value);
	}

	public Cursor sapXepNhaTro(String tieuChi) {
		SQLiteDatabase db = this.getWritableDatabase();
		return db.rawQuery("select * from " + tableNhaTro + " order by "
				+ tieuChi, null);
	}
